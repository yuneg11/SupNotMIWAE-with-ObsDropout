...
class GRUAutoCell(DropoutRNNCellMixin, base_layer.BaseRandomLayer):
    def __init__(
        ...
        **kwargs,
        ...
        self.state_size = [
            tf.TensorShape([None, self.hidden_units]),  # h
            tf.TensorShape([None, self.output_units]),  # y
        ]
        self.output_size = [
            tf.TensorShape([None, self.output_units]),  # y
            tf.TensorShape([None, self.output_units]),  # mu
            tf.TensorShape([None, self.output_units]),  # sigma
        ]
    ...
    def call(self, inputs, states, training=None):
        ...
        if self.implementation == 1:
            ...
            z = self.recurrent_activation(backend.expand_dims(x_z, axis=1) + recurrent_z + y_tm1_z)
            r = self.recurrent_activation(backend.expand_dims(x_r, axis=1) + recurrent_r + y_tm1_r)
            ...
            hh = self.activation(backend.expand_dims(x_h, axis=1) + recurrent_h + y_tm1_h)
        else:
            if 0.0 < self.dropout < 1.0:
            ...
            matrix_x = backend.expand_dims(matrix_x, axis=1)
            ...
            z = self.recurrent_activation(x_z + recurrent_z + y_tm1_z)
            r = self.recurrent_activation(x_r + recurrent_r + y_tm1_r)
            ...
            hh = self.activation(x_h + recurrent_h + y_tm1_h)
        ...
        # return (y, mu, sigma), (h, mu)
        return (y, mu, sigma), (h, y)
...
class GRUAuto2(DropoutRNNCellMixin, RNN, base_layer.BaseRandomLayer):
    ...
    def call(self, inputs, mask=None, training=None, initial_state=None, n_samples=1):
        ...
        if initial_state is None:
            initial_state = rnn_utils.generate_zero_filled_state(
                tf.shape(inputs)[0],
                state_size=[
                    tf.TensorShape([n_samples, self.cell.hidden_units]),
                    tf.TensorShape([n_samples, self.cell.output_units]),
                ],
